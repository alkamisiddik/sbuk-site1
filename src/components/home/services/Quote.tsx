...
export function AnimatedQuote({ quote }: { quote: Quote }) {
  ...
  // On mobile: play animation when entering view, hide when leaving
  useEffect(() => {
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row items-center gap-10 md:gap-13">
        {/* === Image Section (animates from left) === */}
        <div
          className={`relative flex-shrink-0 overflow-hidden transition-all duration-700 ease-out ${isHovered
            ? "translate-x-0 opacity-100"
            : "-translate-x-[100vw] opacity-0"
            }`}
        >
          <div className="relative w-[300px] sm:w-[340px] h-[450px] sm:h-[518px] pt-[25px]">
            <Image
              src={quote.imgSrc || "/placeholder.svg"}
              alt={quote.imageAlt}
              fill
        ...
        <div
          className={`bg-white flex relative self-center flex-col items-start justify-center gap-5 md:ml-4 mb-2 transform transition-all duration-700 ease-out ${isHovered
            ? "translate-x-0 opacity-100"
            : "translate-x-full opacity-0"
            }`}
        >
          <p className="font-light font-helvetica text-black text-xl lg:text-5xl md:text-3xl sm:text-2xl md:leading-[131%] relative">
            <span className="relative inline-block align-text-top -mt-1 mr-2">
              <svg
                width="24"
                height="24"
                viewBox="0 0 39 29"
            ...
            {quote.start} <span className="font-medium text-secondary-500">{quote.highlight}</span>{quote.end}
            <span className="relative inline-block align-text-top -mt-1 ml-2">
              <svg
                width="24"
                height="24"
                viewBox="0 0 39 29"
          ...
          <p className="relative w-fit font-helvetica font-medium text-primary-500 text-base tracking-[0] leading-[160%] whitespace-nowrap md:pt-5 uppercase">
            —&nbsp;&nbsp;{quote.title}
          </p>
        </div>
      </div>
