...
export function SbukFooter({ footerData }: FooterProps) {
    const socialLinks = [
        { icon: LucideInstagram, href: footerData.socialLinks.instagram, label: "Instagram" },
    ...
    return (
        <footer className="bg-primary-900 text-white relative pb-10">
            <div className="global-px py-10 md:py-[80px]">
                {/* --- Main Grid --- */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-10 lg:gap-[60px]">
                    {/* Logo + Description */}
                    <div className="lg:col-span-2 md:w-[350px]">
                        <div className="flex flex-col">
                            <div className="relative w-[107px] h-[50px]">
                                <Image
                            ...
                            <div className="mt-1">
                                <div className="font-family-helvetica font-medium text-[9px] leading-[160%] uppercase">
                    ...
                    {/* --- Service Links --- */}
                    <div className="lg:col-span-4 grid grid-cols-1 md:grid-cols-4 gap-8 sm:gap-10">
                        {footerData.footerItems.map((services, index) => (
                            <div key={index}>
                                <h3 className="font-medium font-family-helvetica text-lg md:text-[22px] leading-[120%] mb-4 md:mb-6">
                                    {services.title}
                                </h3>
                                ...
                                <ul className="space-y-3 md:space-y-4">
                                    {services.items.map((service, idx) => (
                                        <li key={idx}>
                                            <Link
                                                href={service.href}
                                                className="group inline-flex items-center gap-2 text-sm md:text-base font-normal font-family-helvetica leading-[160%] transition-all duration-200 hover:underline hover:underline-offset-8 hover:text-secondary-500 hover:-translate-y-[2px]"
                                            >
                                                {service.label}
                                                <ArrowUpRight className="opacity-0 -translate-x-1 transition-all duration-200 group-hover:opacity-100 group-hover:translate-x-0" />
                                            </Link>
                                        </li>
                                    ))}
                                </ul>
                ...
                {/* Divider */}
                <div className="pt-10">
                    <div className="border border-neutral-600 opacity-50" />
                </div>
                    ...
                    <div className="flex gap-3">
                        {socialLinks.map((social, index) => (
                            <a
                                ...
                                className="w-10 h-10 md:w-11 md:h-11 rounded-full bg-[#E3A91F] flex items-center justify-center hover:bg-[#E3A91F]/90 transition-colors"
                            >
                                <social.icon className="w-5 h-5 text-primary-900" strokeWidth={1.5} />
                            </a>
                        ))}
                    </div>
                </div>
