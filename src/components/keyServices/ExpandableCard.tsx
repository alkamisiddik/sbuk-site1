...
export default function ExpandableCard({
  ...
  return (
    ...
    >
      {/* Background image */}
      <div className="relative w-full aspect-[16/16] md:aspect-[16/9]">
        <Image
          src={image}
          alt="Key Service"
          fill
      ...
      <h2
        className={`font-family-helvetica absolute bottom-6 md:bottom-8 left-1/2 -translate-x-1/2 w-[90%] text-white text-lg md:text-xl lg:text-[22px] font-bold uppercase text-center drop-shadow-lg ${isActive ? "hidden" : "inline"
          }`}
      >
        {title}
      </h2>
      ...
      <AnimatePresence>
        {isActive && (
          ...
          >
            <div className="font-family-helvetica mt-12 md:mt-16 lg:mt-18 mx-6 md:mx-8 lg:ml-13 lg:mr-10 pb-6">
              {/* Title */}
              <div className="font-medium text-2xl md:text-3xl lg:text-[34px]">{title}</div>
              ...
              <div className="flex flex-col lg:flex-row justify-between gap-3 mt-6 lg:mt-8">
                {boxDetails.map((box, idx) => (
                  ...
                  >
                    <CardHeader>
                      <CardTitle className="font-family-helvetica text-base md:text-lg">{box.boxTitle}</CardTitle>
                    </CardHeader>
                    <CardContent>
              ...
              <Button className="mt-4 lg:mt-6 mb-2 h-10 w-30 p-3 text-sm md:text-base font-normal bg-highlight-500 text-white hover:bg-transparent/40 hover:border hover:border-highlight-500 hover:text-highlight-500 group">
                {buttonText}
